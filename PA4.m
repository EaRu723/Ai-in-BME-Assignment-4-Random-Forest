...
D = M_training;
k = 4;              % you can set the k value here
bag = bagging(D, k);
...
for i=1:size(Y)
  [d(i), a] = forestPredict(t, X(i,:));
end
cor_train = d' == Y;
correctTrain = (length(find(cor_train==1)) ./length(Y))*100; %you need to change this
...
for i=1:size(Ytest)
  [e(i), b] = forestPredict(t, Xtest(i,:));
end

cor_test = e' == Ytest;
correctTest = (length(find(cor_test==1)) ./length(Ytest))*100; %you need to change this
